...
export default withCommonAdaptorOptions(
  task("adaptor-enable-layerzero", "Enable or disable LayerZero integration on BridgeAdaptor").addOption({
    name: "enabled",
  ...
  .setInlineAction(async (args: Args, hre: any) => {
    ...
    const tx = await adaptor.setLayerZeroEnabled(enabled, getTxOverrides(args));
    await confirmTx(tx, "setLayerZeroEnabled");
    console.log("LayerZero integration", enabled ? "enabled" : "disabled");
  })
  .build();
